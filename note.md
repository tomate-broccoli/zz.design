...
- A.
  ...
  - 実装においては、エンティティを汚染したくないのでドメインサービスに検査処理を定義したいが、
    ...
    でもそうすると同じような処理があっちこっちに作成されることになるので、やはり(参照に限って)ドメインサービスに実装することになりそう...
...
- A.
  ...
  - いや、こういう切っても来れないものはエンティティ内に定義すべきでは？
  - 定義(設計)段階では、対象のコマンドの集約にロジックを追加する.
...
- A.
  ...
  - 実装においては、エンティティを汚染したくないのでドメインサービスに処理を定義したいが、
    ドメイン内でリポジトリ処理をしたくない...こういうのはアプリケーションサービスにさせたい.
    でもそうすると同じような処理があっちこっちに作成されることになるので、やはり(参照に限って)ドメインサービスに実装することになりそう...
...
- Q.アプリケーションサービスの洗出しは？
- A.
  - アクターのコマンドがそれ(ユースケース)の可能性が高い.
  - 分類でタグ付けされているコマンド. (例: .class(usecase))
